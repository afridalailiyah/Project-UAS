    ...
    <section class="content">
      <div class="container-fluid">
        <div class="not-printed row justify-content-center">
        	<div class="col-lg-12 ml-4">
        		...
        		<form method="post">
        			...
        			<div class="form-group">
        				<button type="submit" name="btnLaporanPengeluaran" class="btn btn-primary">Laporan Data</button>
        			</div>
        		</form>
        	</div>
        </div>
	        	...
	        	<div class="col-lg m-1">
	        		...
	        		<div class="table-responsive">
	        			<table class="table table-bordered table-hover">
	        				...
	        				<tbody>
	        					<?php $i = 1; ?>
	        					<?php foreach ($sql as $dp): ?>
	        						<tr>
	        							<td><?= $i++; ?></td>
	        							<td><?= ucwords(htmlspecialchars_decode($dp['nama_siswa'])); ?></td>
	        							<?php if ($dp['minggu_ke_1'] == $dp['pembayaran_perminggu']): ?>
					                      <td class="text-success"><?= number_format($dp['minggu_ke_1']); ?></td>
					                    <?php else: ?>
					                      <td class="text-danger"><?= number_format($dp['minggu_ke_1']); ?></td>
					                    <?php endif ?>

					                    <?php if ($dp['minggu_ke_2'] == $dp['pembayaran_perminggu']): ?>
					                      <td class="text-success"><?= number_format($dp['minggu_ke_2']); ?></td>
					                    <?php else: ?>
					                      <td class="text-danger"><?= number_format($dp['minggu_ke_2']); ?></td>
					                    <?php endif ?>

					                    <?php if ($dp['minggu_ke_3'] == $dp['pembayaran_perminggu']): ?>
					                      <td class="text-success"><?= number_format($dp['minggu_ke_3']); ?></td>
					                    <?php else: ?>
					                      <td class="text-danger"><?= number_format($dp['minggu_ke_3']); ?></td>
					                    <?php endif ?>

					                    <?php if ($dp['minggu_ke_4'] == $dp['pembayaran_perminggu']): ?>
					                      <td class="text-success"><?= number_format($dp['minggu_ke_4']); ?></td>
					                    <?php else: ?>
					                      <td class="text-danger"><?= number_format($dp['minggu_ke_4']); ?></td>
					                    <?php endif ?>
	        						</tr>
	        					<?php endforeach ?>
	        				</tbody>
        ...
        <?php if (isset($_POST['btnLaporanPengeluaran'])): ?>
        	<hr class="not-printed">
        	<button onclick="return print()" class="not-printed btn btn-success"><i class="fas fa-fw fa-print"></i> Print</button>
			<a href="export_excel2.php" class="not-printed btn btn-primary"><i class="fas fa-fw fa-file-excel"></i> Export Excel</a>

        	<div class="row m-1 mb-0">
	        	<div class="col-lg m-1">
	        		<h2 class="text-center mb-3 mt-2">Laporan Data Ibu Hamil</h2>
